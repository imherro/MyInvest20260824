...
test("range position reaches the top of its recent range", () => {
  assert.equal(calculateRangePosition([8, 9, 10, 11, 12], 5), 1);
});

test("range position uses the latest close within its recent range", () => {
  assert.equal(calculateRangePosition([8, 12, 9, 11, 10], 5), 0.5);
});

test("range position is unavailable with insufficient or flat history", () => {
  assert.equal(calculateRangePosition([8, 9, 10, 11], 5), null);
  assert.equal(calculateRangePosition([10, 10, 10, 10, 10], 5), null);
});
