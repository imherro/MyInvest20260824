...
## 2026-08-25 — V5 Task 2 首页 A 股异动事实原因

首页“今日重点关注”在既有的当日绝对涨跌幅前五选取完成后，只收集其中 A 股代码并使用一次批量异动原因请求（最多 5 个）；ETF 不请求该接口，也不新增列表列、筛选、排序、独立页面、缓存、数据库或推送。响应按精确 `thscode` 映射回原有标的名称区域，若有记录则展示上游的异动标签与原因原文；`keyword_list` 不展示。成功响应的异动数据时间独立显示，未与行情快照或历史日线时间混用。

异动请求独立捕获：接口整体失败不会影响首页行情、重点指标和历史日线，受影响的 A 股行只显示“异动原因暂不可用”和非敏感错误代码；业务成功但没有该股记录时如实显示“未返回异动原因”，不将空数组解释为“今日无异动”。开工前用 `600350.SH,600361.SH` 实际预检，业务 `code=0`、`item=[]`，确认空结果是正常可展示分支；没有打印或提交 API Key、请求头或原始响应。

## 2026-08-25 — V5 Task 1 A 股财务体检

V5 从“研究记录管理”进入“研究证据”方向，首个任务只在 `/stock/[thscode]` 增加 A 股财务体检。开工前以 `600350.SH` 对利润表、资产负债表、现金流量表（均为 `period=quarterly&limit=8`）和对应 `2026-1` 财务指标做最小权限验证，四类接口均返回业务 `code=0`；API Key 未打印、未写入代码或 Git。

`lib/hithink.ts` 继续复用现有 server-only `hithinkFetch`，仅新增四个财务请求函数及窄字段校验。三张报表先按 `period_end_ms` 内连接，取最新共同报告期再请求格式为 `YYYY-[1-4]` 的财务指标；不按数组下标拼接，不把 `null` 补为 0。详情页展示最新共同报告期的营收、归母净利润、同比增长率、销售毛利率、加权平均净资产收益率、经营现金流、净利润现金含量、总资产、总负债和资产负债率，并显示最多 8 个对齐报告期的证据表。

财务请求独立于核心行情和历史日线：任何财务错误只在财务区域显示“财务数据暂不可用”和非敏感错误代码，行情、研究记录和 K 线仍按原有路径工作。页面明确列出报告期、披露日期、CNY 与数据源；不生成财务评分、投资结论、目标价或单季环比解释。ETF、港股、研究记录 DuckDB、页面导航、依赖和 API 路由均未修改。

### Verification

- `600350.SH` 真实页面显示 2026 Q1、披露日 2026-04-30、CNY、四类财务事实与 8 期按报告期末对齐的证据表；指标值来自接口实际返回字段。
- 390×844 下，四类事实卡片改为单列，页面 `body` 与 `documentElement` 均未超出视口宽度；历史证据表仅在自身容器内横向滚动。
- `npm test`（12 项）、`npm run build` 与 `git diff --check` 通过；`/stock/600350.SH` 在生产服务中返回 HTTP 200，服务继续监听 `0.0.0.0:8030`。
...
本修复只改变容器宽度，不改变行情请求、行业排序、成分股字段、上涨宽度、研究记录、依赖或路由结构。
